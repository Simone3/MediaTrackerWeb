...
- **An action that changes what the media items list queries must reset `currentPage` to 0.** The list is paginated and the page index deliberately survives a refetch, so that a write does not cost the user their place; a new filter, search or mode that keeps the old index silently shows page 5 of a different query instead ([§10.2](docs/technical/10-features.md#102-media-items-list)).
...
- **A container binds to Redux through `useContainerInput` / `useContainerOutput` from `app/redux/hooks.ts`, never `connect`.** It is a function component that selects the input props, builds the output callbacks and renders one presentational component. Anything the selector derives rather than reads straight out of the state has to be memoized, or the container re-renders on every action ([§11.1](docs/technical/11-interface.md#111-containers)).
- **Reuse the shared presentational building blocks before creating a screen-specific duplicate**: `generic/responsive-action-menu`, `generic/entity-management-screen`, `generic/entity-management-list`, `generic/entity-details-frame`, `generic/same-name-confirmation`, `generic/color-picker`, `generic/input`, `generic/select`, `generic/textarea`, `generic/decimal-input`, `generic/field-error`, `generic/pagination`, `generic/responsive-header-button`, and the own-platform `icon-registry.ts` ([§11.3](docs/technical/11-interface.md#113-shared-building-blocks)).
...
- **For responsive JS behaviour, reuse `MOBILE_LAYOUT_BREAKPOINT` and `useIsMobileLayout()` from `app/utilities/layout.ts`.** Do not introduce a new hardcoded breakpoint.
