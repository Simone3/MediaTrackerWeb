...
**`status` is derived, not stored.** `mediaItemUtils.buildStatusLabel` computes it from `completedOn`, `active`, `markedAsRedo` and `releaseDate`, in that precedence, and it is the only place the front end resolves the label: the mapper calls it when an API media item arrives, and the inline list update calls it again after changing the stored fields, so a row cannot be labelled one way by an inline update and another way by the refetch that follows it. **The back end applies the same five branches** to bucket the stats backlog, because shipping enough per-item data to bucket on the client would defeat the point of an aggregate ([§10.9](10-features.md#109-media-items-stats)): the rule therefore lives in two places, held together by a table of cases duplicated in both test suites ([§15.11](15-invariants-and-pitfalls.md#1511-the-media-item-status-rule-lives-on-both-sides), [back end §9.6](../../../back-end/docs/technical/09-controllers.md#96-mediaitementitycontroller)). One consequence is accepted rather than solved: `UPCOMING` is decided against the browser's clock here and against the server's there, so an item releasing today can be counted differently by the two.
