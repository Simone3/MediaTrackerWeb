...
| `generic/pill-button` | |
| `generic/input`, `generic/select`, `generic/textarea`, `generic/clearable-input`, `generic/decimal-input` | The form controls. `generic/decimal-input` is the one a decimal field needs: a text input with `inputMode='decimal'`, since a `number` input loses a value typed with the comma a mobile decimal keypad offers ([§10.5](10-features.md#105-groups)) |
