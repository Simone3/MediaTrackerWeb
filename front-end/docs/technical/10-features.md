...
- the media-item form's order-in-group field is a **text input with `inputMode='decimal'`**, not a `number` input: the mobile decimal keypad types the locale decimal separator, and a `number` input reports an empty value for anything containing a comma, so the typed order silently vanished. `DecimalInputComponent` keeps the raw text while it is being typed, ignores anything that is not digits plus at most one separator, and syncs `,` and `.` alike to Formik as a number. The bounds are the schema's alone ([§8.3](08-domain-model.md#83-group)): an out-of-range order disables Save and says why, once the field has been left ([§11.6](11-interface.md#116-form-validation-feedback))
