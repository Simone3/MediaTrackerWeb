...
- every call logs its method, collection, conditions and elapsed time when enabled ([§14.3](14-logging.md#143-query-logging))
...
**Go through `QueryHelper` rather than reaching for Mongoose directly.** One-off access styles per controller lose the collation, the populate flags and the query logging, all silently — and the query logging has no other chokepoint since the `mongoose.set('debug')` hook was dropped ([§14.3](14-logging.md#143-query-logging)).
