...
- Linting now runs on ESLint `9` flat config through the root `eslint.config.js`; the npm lint script checks both `app/**` and `tests/**`, while a test-only override keeps Jest globals enabled, adds focused Jest guardrails (`no-focused-tests`, `valid-expect`, duplicate-title checks, etc.), and still relaxes a few app-focused strict TypeScript safety/deprecation rules so mocks do not drown the suite in noise.
