...
- A follow-up stylesheet token audit removed six more dead semantic `:root` variables from `app/web/styles.css` (`--color-background-app-elevated`, the old sidebar background pair, `--color-screen-glow-secondary`, and the unused nav/tooltip shadow colors) after confirming they had no runtime `var(--...)` references left in app code or selectors.
...
- A later translation audit removed ten dead `lang-en.json` entries, including unused shared form/section labels plus stale media-item and TV-show-season strings. The three smoke tests that had referenced removed section labels now assert the absence of any level-2 section headings instead of pointing at translations the runtime no longer owns.
...
- Native date inputs on iOS Safari could render wider than their grid cell, making media-item detail date fields look oversized and overflow to the right on mobile.
  ...
  - Relevant files:
    ...
    - `app/web/styles.css`
    - `tests/clearable-input.test.tsx`
...
- The TV-show details summary line on web no longer says `1 seasons` for a single season.
  ...
  - Relevant files:
    ...
    - `app/components/presentational/media-item/details/form/view/tv-show.tsx`
    - `tests/media-item-details.smoke.test.tsx`
- Save feedback on details forms keeps the existing overlay, but the spinner is now centered on the visible viewport instead of the full page height.
  - Correct behavior on web now:
    - category, group, own-platform, and media-item details still use the shared loading overlay during saves
    - media-item details still use that same overlay for catalog fetches and other related async form actions
    - the non-fullscreen loading overlay is now `position: fixed` with a higher stacking order, so its spinner stays visible in the middle of the current viewport even on long scrolled pages and above the sticky header
    - focused smoke coverage asserts the loading overlay is present for both the shared entity-details frame and the media-item details screen while Save stays disabled
  - Relevant files:
    - `app/components/presentational/generic/loading-indicator/index.tsx`
